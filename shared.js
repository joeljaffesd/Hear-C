...
// Based on https://github.com/binji/wasm-clang/blob/master/shared.js
// Modified to add compileAndGetWasm / linkForAudio for Hear-C client-side audio compilation.
...
class App {
  ...
  canvas_requestAnimationFrame() {}
  canvas_createImageData(w, h) { return -1; }
  ...
  canvas_lineTo(...args) {}
  canvas_measureText(text, text_len) { return 0; }
  canvas_moveTo(...args) {}
...
class API {
  ...
  // Compile and link user audio code, returning the WASM bytes as ArrayBuffer.
  async compileAndGetWasm(contents) {
    const input = 'main.cpp';
    const obj = 'main.o';
    const wasm = 'main.wasm';
    await this.compile({input, contents, obj});
    await this.linkForAudio(obj, wasm);
    const bytes = this.memfs.getFileContents(wasm);
